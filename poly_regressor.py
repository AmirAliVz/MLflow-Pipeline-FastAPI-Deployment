...
parser = argparse.ArgumentParser(description='Parse the parameters for the polynomial regression: number of alphas & polynomial degrees')

parser.add_argument("--num_alphas", type=int, default=20,
                    help="Number of Lasso penalty increments")
...
# Printing the hyperparameters Polynomial degree & Number of alphas
...
logname = os.path.join(LOG_DIR, "polynomial_regression.txt")
...
def create_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df2.loc[df2['DEPARTURE_DELAY'] >= 60, 'DEPARTURE_DELAY'] = np.nan
    df2 = df2.dropna(how = 'any')
    #_________________
    # formating times
...
f = open(os.path.join(LOG_DIR, "airport_encodings.json"),"w")
...
avg_delay_min = np.mean(np.abs(result - Y_test))  # average absolute error in minutes
logging.info('Test Data MSE = {}'.format(round(score, 2)))
logging.info("Predictions using test data successful")

logging.info('Test Data average delay = {:.2f} min'.format(float(avg_delay_min)))

# export final model
pickle.dump(ridgereg, open(os.path.join(LOG_DIR, 'finalized_model.pkl'), 'wb'))
...
plt.savefig(os.path.join(LOG_DIR, "model_performance_test.jpg"),dpi=300)
...
with mlflow.start_run(run_name = "Final Model - Test Data", nested=True):
    ...
    # 1. Log files from import_format_data and filter_clean_data (when those scripts write them)
    if os.path.exists("logs/import_format_data_log.txt"):
        print("Import format data log file exists")
        mlflow.log_artifact("logs/import_format_data_log.txt")
    if os.path.exists("logs/filter_clean_data_log.txt"):
        print("Filter clean data log file exists")
        mlflow.log_artifact("logs/filter_clean_data_log.txt") 

    # 2. Input parameters (alpha and order) for final regression on test data
    mlflow.log_param("best_alpha", parameters[0] / 10)
    mlflow.log_param("polynomial_order", order)

    # 3. Performance plot
    mlflow.log_artifact("logs/model_performance_test.jpg")

    # 4. Model performance metrics: MSE and average delay in minutes
    mlflow.log_metric("Test_MSE", score)
    mlflow.log_metric("Test_avg_delay_min", float(avg_delay_min))

    # Additional artifacts
    mlflow.log_artifact("logs/polynomial_regression.txt")
    mlflow.log_artifact("logs/airport_encodings.json")
    mlflow.log_artifact("logs/finalized_model.pkl") 
